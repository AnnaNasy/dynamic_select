...
class LoadInfo():
	...
	_access_time_info_path = "/usr/local/nginx/logs/access.log"
	...
	def get_cur_req_numbers(self):
		with os.popen("cat %s |wc -l" % self._access_time_info_path) as f:
			return f.read().strip()

	def clear_access_log(self):
		os.system("echo '' > %s" % self._access_time_info_path) #把log清空，避免干扰
	...
	def _read_last_line_time(self, f):
		offs = 500 #保证读到3行
		
		try:
			f.seek(-offs, 2)
		except OSError as e:
			#print("文件不够长 seek失败", e)
			return None
		lines = f.readlines()
		if not lines:
			#print("没有读到内容")
			...
			cur_flag = lines[-1].strip().split()[-6]
			#print("上一个flag = %s 当前falg = %s" % (self._flag, cur_flag))
			if self._flag == cur_flag:
				return None #读到的数据没有变化
			else:
				self._flag = cur_flag
			request_times = 0.0
			n = len(lines)
			for line in lines:
				try:
					line_time = float(line.strip().split()[-1])
					request_times += line_time
				except ValueError as e:
					print("ValueError ", e)
					return None

			return (request_times/n)


	def get_request_time(self):
		request_time = self._read_last_line_time(self._f_time_usage)
		return request_time
	...
	def _open_files(self, files=None):
		if not files:
			...
			self._f_time_usage = open(self._access_time_info_path, 'rb')
			if not self._f_cpu_usage or not self._f_mem_usage or not self._f_net_usage or not self._f_time_usage:
				print("初始化打开文件失败")
				exit(1)
		else:
	...
	def get_net_usage_info(self):
		...
		pnet = 100 * (numbers/time_width)/(1000000*band_width)
