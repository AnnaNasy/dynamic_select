...
make && make install  
安装完成之后 cp php.ini-production /usr/local/php/etc/php.ini 创建php.ini文件


启动php-fpm
> /usr/local/php/etc$ cp php-fpm.conf.default php-fpm.conf  生成配置文件  
/usr/local/php/etc/php-fpm.d$ cp www.conf.default www.conf  生成配置文件  
启动php-fpm /usr/local/php/sbin/php-fpm  

> 报错[17-Jun-2018 08:36:03] ERROR: [pool www] cannot get gid for group 'nobody'  
[17-Jun-2018 08:36:03] ERROR: FPM initialization failed  
修改/usr/local/php/etc/php-fpm.d/www.conf，把nobody改成www-data  
